...
namespace SyntaxAnalyzer
{
	public class LexicalAnalyzer
	{
		...
		public string StartLexicalAnalyzer(string programStr)
		{
			...
			{
				if (programStrArr[i] == "end" && i < programStrArr.Length-1 && programStrArr[i + 1] == ".")
				{
					_arrResult.Add($"[0, {CheckServiceWord(programStrArr[i])}]");
					_arrResult.Add($"[1, {CheckSeparator(programStrArr[i + 1])}]");
					hasEnd = true;
					break;
				}
				else if(programStrArr[i] == "end" && i == programStrArr.Length - 1)
				{
			...
			_form.SetTableNumbers(_numbers);
		...
		public int CheckIdentificator(string str)
		{
			...
			else
			{
				...
				{
					_form.identificators.Add(str); // выводим на форму лист индентификаторов.
					if (_identificators.Count == 0)
					{
						_identificators.Add(new Identificator(0 , str));
						return _identificators.Last().id;
					}
					else
					{
						int lastIndex = _identificators.Last().id;
		...
		public int CheckNumber(string str)
		{
			if (CheckNumeral(str))
			{
				...
				if (NumCC != '-')
				{
					...
					int numberId = _numbers.FindIndex(x => x.number == binaryNum);
					if (numberId == -1)
					{
						...
						if (_numbers.Count == 0)
						{
							_numbers.Add(new Number(0, binaryNum));
							return _numbers.Last().id;
						}
						else
						{
							int lastIndex = _numbers.Last().id;
							_numbers.Add(new Number(lastIndex + 1, binaryNum));
							return _numbers.Last().id;
						}
					}
					else
		...
		public bool CheckExponentialFormNumber(string str)
		{
			if (((str[0] >= '0' && str[0] <= '9') || str[0] == '.') && (str.Contains('e') || str.Contains('E')))
			{
				...
				for (int i = 1; i < str.Length; i++)
				{
					...
					if (str[i] == 'e' || str[i] == 'E')
					{
						if (indexE || indexPlusOrMin || str[i-1] == '.')
						{
							return false;
						}
						else
